# Документация по SQL, Backend (Python) и Frontend (React)

## 1. SQL-скрипт `sem2_point1.sql`

### 1.1. Назначение скрипта
Скрипт полностью поднимает учебную схему продаж: справочники магазинов/продавцов/покупателей, таблицу продаж, журнал изменений, CRUD-пакет и пакет инструментов журнала. Скрипт можно запускать повторно — он сначала удаляет объекты, затем создаёт их заново.
